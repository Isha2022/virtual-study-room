...
describe("GroupStudyHeader", () => {
  ...
  /*
  it("calls leaveRoom on pagehide event", async () => {
    const leaveRoomMock = jest
      .fn()
      .mockResolvedValueOnce({ participantsList: [{}] }); // Only one participant;

    jest.spyOn(console, "log").mockImplementation(() => {});
    jest.spyOn(React, "useCallback").mockImplementationOnce((fn) => {
      // Return both the mock and the original function
      leaveRoomMock.mockImplementation(fn);
      return leaveRoomMock;
    });
    ...
    expect(leaveRoomMock).toHaveBeenCalled();
  });

  it("renders without room state", () => {
...
describe("Spotify Button Handlers", () => {
  beforeEach(() => {
  ...
  it("should close the Spotify dialog when handleClose is called", () => {
    ...
    // Get the close button (assuming it's the button in DialogTitle)
  ...
  it("should toggle floating music player visibility when handleOpenMusicButton is called", async () => {
    // Render the component
    render(<GroupStudyHeader />);

    // Open the Spotify dialog first
    fireEvent.click(screen.getByAltText("Music"));

    // Click the "Switch to Free Tracks" button to open floating player
    const switchButton = screen.getByText("Switch to Free Tracks");
    await act(async () => {
      fireEvent.click(switchButton);
    });

    // Verify floating player is rendered
    expect(screen.getByText("FloatingMusicPlayer")).toBeInTheDocument();

    // Re-open the Spotify dialog (which should close the floating player)
    await act(async () => {
      await fireEvent.click(screen.getByAltText("Music"));
    });

    // Verify floating player is now closed
    expect(screen.queryByText("FloatingMusicPlayer")).toBeInTheDocument();

    // Verify Spotify dialog is open again
    expect(screen.getByText("Spotify Player")).toBeInTheDocument();
  });

  it("should close Spotify dialog when opening floating music player", () => {
    render(<GroupStudyHeader />);
    ...
    fireEvent.click(screen.getByText("Switch to Free Tracks"));

    // Spotify dialog should close
    expect(screen.queryByText("Spotify Player")).not.toBeInTheDocument();

    // Floating player should open
    expect(screen.getByText("FloatingMusicPlayer")).toBeInTheDocument();
  });

  it("should maintain proper state when toggling between music options", () => {
    render(<GroupStudyHeader />);

    // Initial state - both closed
    expect(screen.queryByText("Spotify Player")).not.toBeInTheDocument();
    expect(screen.queryByText("FloatingMusicPlayer")).not.toBeInTheDocument();
